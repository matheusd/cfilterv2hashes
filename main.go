...
func main() {
	...
	if opts.TestNet {
		network = "testnet3"
		wantNet = wire.TestNet3
	}

	// Ensure we're connected to a dcrd in the same network as the expected
	// one.
	dcrdNet, err := client.GetCurrentNet()
	if err != nil {
		log.Fatal(err)
	}
	if dcrdNet != wantNet {
		log.Fatalf("dcrd (%s) not on the expected network (%s)", dcrdNet, wantNet)
	}
	...
	// people can verify them easily. Also store the hashes in memory to
	// calculate the merkle tree for the full set.
	target := int64(opts.TargetHeight)

	// If we should report the final stats, then track the hash for the
	// full cfilter dataset and the hashes of individual cfilters so we can
	// calculate a merkle tree root at the end.
	var hasher hash.Hash
	var hashes []chainhash.Hash
	if opts.Progress {
		log.Printf("Generating up to block %d for %s", opts.TargetHeight, network)
		hasher = blake256.New()
		hashes = make([]chainhash.Hash, target+1)
	}
	...
	for h := int64(0); h <= target; h += step {
		...
		for i, cf := range cfilters {
			...
			// If we're not tracking stats, then continue.
			if !opts.Progress {
				continue
			}
			...
			// Dataset hash and merkle root hash tracking.
			hasher.Write(cf.Bytes())
			hashes[cfheight] = cf.Hash()
